	...
	if($_POST && $request->element('doo') == 'submit') { # if form is submitted
		...
		if($validate['invalid']) {	# data input is not in valid form
			...
			if(!$validate['invalid']) {
				...
				// var_dump($request->element('chkStatus'));
				// exit();
				//if($request->element('chkStatus') != 1){
					// $id1=$id+1;
					
					// $fields1 = array('status_bill'	=> $request->element('chkStatus'));
					// $expenditureId = $expenditures->updateData($fields,$id);
					// $expenditureId1 = $expenditures->updateData($fields1,$id1);
				...
				header('location:'.'/'.ADMIN_SCRIPT."?op=budget&act=billrevenue&mod=edit&id=$id&rcode=7");
			}else{
