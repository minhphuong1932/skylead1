...
$listTabs = array($amessages['list_item'] => $tabLink.'&mod=list',
				...
				//$amessages['list_category'] => $tabLink.'&mod=list',
				//$amessages['list_weight'] => $tabLink.'&mod=list',
...
$url = '/'.ADMIN_SCRIPT."?op=managematerial&act=product&mod=list&doo=$do&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pId=$pId&pg=%d";
...
# Link
$link = '/'.ADMIN_SCRIPT."?op=managematerial&act=product&mod=list&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pId=$pId&pg=$page";
...
if($_POST) {
	switch($do) {
		...
		case 'changeposition':
			//$userInfo->checkPermission('product','edit');
			checkPermission();
			$cat_ids = $request->element('cat_ids');
			$names = $request->element('names');
			$warnings = $request->element('warnings');
			if($names) {
				...
				foreach ($cat_ids as $key=>$value) {
					$products->changeCatId($key,$value);
				}
				$result_code = 4;
				
				
				foreach ($warnings as $key=>$value) {
					$products->changeWarning($key,$value);
				}
				$result_code = 4;
				
				# Operation tracking
				$trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>$amessages['tracking']['change_product_cateid'],'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
			} else $error_code = 5;
			break;
		case 'cleantrash':
			...
			break;		
		case 'cancel':		
			header('location:'.'/'.ADMIN_SCRIPT."?op=managematerial&act=product&mod=list&lang=$lang&ecode=7&pId=$pId");
			exit;
			break;
	}
	header('location:'.'/'.ADMIN_SCRIPT."?op=managematerial&act=product&mod=list&doo=$do&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pg=$page&ecode=$error_code&rcode=$result_code&pId=$pId");
} else {
