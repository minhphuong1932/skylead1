...
Coder: Mai Minh
**************************************************************************/
$userInfo->checkPermission('budgetcurrency','view');
...
if($pId) {
	...
	$topNav[$amessages['list_item']] = '/'.ADMIN_SCRIPT.'?op=budget&act=currency&mod=list';
...
$url = '/'.ADMIN_SCRIPT."?op=budget&act=currency&mod=list&doo=$do&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pId=$pId&pg=%d";
$urls = new Url();
...
# Link
$link = '/'.ADMIN_SCRIPT."?op=budget&act=currency&mod=list&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pId=$pId&pg=$page";
...
if($_POST) {
	switch($do) {
		...
		case 'update':
			...
			$display = $request->element('displays');
			if($positions) {
			...
			if($display) {
				foreach ($display as $key=>$value) {
					$currencies->changeDisplay($key,$value);
				}
				$result_code = 4;
				# Operation tracking
				$trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>$amessages['tracking']['change_currency_display'],'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
			}
			if($decimal) {
		...
		case 'primary':
			...
			break;
		case 'cancel':		
			header('location:'.'/'.ADMIN_SCRIPT."?op=budget&act=currency&mod=list&lang=$lang&ecode=7&pId=$pId");
			exit;
			break;
	}
	header('location:'.'/'.ADMIN_SCRIPT."?op=budget&act=currency&mod=list&doo=$do&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pg=$page&ecode=$error_code&rcode=$result_code&pId=$pId");
} else {
