...
//checkPermission();
$templateFile = 'budgetbillexpenditure.tpl.html';
...
$condition .= " and `type` = 2 and `real_st` = 1";
...
$condition1 .= " and `type` = 2 and `real_st` = 1 and `status`= 1";
...
$url = '/'.ADMIN_SCRIPT."?op=budget&act=billexpenditure&mod=list&doo=$do&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&uid=$uid&type=$type&capitalId=$capitalId&budgetId=$budgetId&status=$status&start=$dateStart&end=$dateEnd&ecode=$error_code&pg=%d";
$urls = new Url();
...
# Link
$link = '/'.ADMIN_SCRIPT."?op=budget&act=billexpenditure&mod=list&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&uid=$uid&type=$type&capitalId=$capitalId&budgetId=$budgetId&status=$status&start=$dateStart&end=$dateEnd&ecode=$error_code&pg=$page";
...
if($_POST) { 
	switch($do) {
		...
		case 'cleantrash':
			...
			break;
		case 'cancel':
			header('location:'.'/'.ADMIN_SCRIPT."?op=budget&act=billexpenditure&mod=list&lang=$lang&ecode=7&uId=$uId");
			exit;
			break;
	}
	header('location:'.'/'.ADMIN_SCRIPT."?op=budget&act=billexpenditure&mod=list&doo=$do&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&capitalId=$capitalId&budgetId=$budgetId&status=$status&start=$dateStart&end=$dateEnd&pg=$page&ecode=$error_code&rcode=$result_code");
} else {
}
