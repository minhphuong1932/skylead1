...
if(!$productInfo) {
	...
	if($userInfo->isSiteStaff()){
		$proCateInfo = $productCategories->getObject($productInfo->getCatId(),'cat_id');
		if($proCateInfo->getId() != $userId)	header("location: /index.php?op=accessdenied");
	}
	...
	if($_POST && $request->element('doo') == 'submit') { # if form is submitted
		...
		if($validate['invalid']) {	# data input is not in valid form
			...
			$slug = TextFilter::cleanVietnamese($request->element('slug'));
			...
			if(!$validate['invalid']) {
				...
				if($productInfo) {			
					...
					$data = array('store_id' => $storeId,
								  ...
								  'warning' => Filter($request->element('warning')));
					$products->updateData($data,$id);
...
function validateData($request) {
	...
	if($error['INPUT']['name']['error'] || $error['INPUT']['unit']['error'] || $error['INPUT']['slug']['error']) {
		$error['invalid'] = 1;
		return $error;
	}
	$error['invalid'] = 0;
