...
include_once(ROOT_PATH."classes/dao/documenttrackinginfo.class.php");

class DocumentTrackings extends Model {
	public $table;
	public $_db;
	private $store_id;
	...
	public function __construct($store_id = 0, $database = '') {
		...
		$this->table = DB_PREFIX."document_trackings";
		$this->store_id = $store_id;
	}
	public function DocumentTrackings($store_id = 0, $database = '') {
...
*-----------------------------------------------------------------------*/
	public function getObject($value = '0', $key = 'id') {
		...
		if($result) {
			$object = new DocumentTrackingInfo
						(	$result[0]['store_id'],
	...
	public function getObjects($page = 1, $condition = '`id` = 0', $sort = array(), $items_per_page = DEFAULT_ADMIN_ROWS_PER_PAGE) {
		...
		if($results) {
			...
			foreach($results as $key => $result) {
				$objects[] = new DocumentTrackingInfo
								(	$result['store_id'],
									$result['document_id'],
									$result['username'],
									$result['action'],
